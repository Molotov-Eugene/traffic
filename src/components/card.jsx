...
export function Card(props) {
  ...
  const camerasList = cameras.cameras.find((c) => c.parentId === object.id).list;
  ...
  const cameraStatus = (camera) => {
    if (object.status === 'active' || object.status === 'notActive') {
      ...
      return <img className='card-cameras-camera-status' src={statusSvg} onClick={() => cameraStatusHandler(camera)} />
    } else {
  ...
  const allCamerasStatus = () => {
    let inactive = (object.status === 'error' || object.status === 'notConfigured') ? 'inactive' : null;

    if (camerasList.filter((c) => c.status === 'active').length === 0) {
      return (<img className={inactive} src={icons.card.cpStart} onClick={() => allCamerasStatusHandler(object.id, 'notActive')} />);
    } else {
      return (<img className={inactive} src={icons.card.cpPause} onClick={() => allCamerasStatusHandler(object.id, 'active')} />);
    }
  }
  ...
  const cameraStatusHandler = (camera) => {
    const { id, status } = camera;

    cameras.cameras[object.id].list[id].status = status === 'active' ? 'notActive' : 'active';
    updateCameraStatus(cameras);
  }

  const cardImage = () => {
    const [imageSource, setImageSource] = useState(0);
    const cardImage = (object.status === 'error' || object.status === 'notConfigured')
      ? icons.card.stub
      : images.images[camerasList[imageSource].image];

    if (camerasList.length > 1) {
      return (
        <div className='card-image'>
          <img src={cardImage} />
          <div className='card-image-pages'>
            {camerasList.map((c, ind) => {
              return (
                <div className={`card-image-page${imageSource === ind ? ' card-active-page' : ''}`} data-tooltip={`Выбрать изображение с ${c.name}`} key={c.id} onClick={() => setImageSource(ind)}>
                  {ind + 1}
                </div>
              )
            })}
